...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        NotificationCenter.default.addObserver(forName: Notification.Name("AgoraChatLogout"), object: nil, queue: .main) { [weak self] _ in
            self?.setRootViewController(scene: windowScene)
        }
        NotificationCenter.default.addObserver(forName: Notification.Name("AgoraChatLogin"), object: nil, queue: .main) { [weak self] _ in
            self?.setRootViewController(scene: windowScene)
        }
        self.setRootViewController(scene: windowScene)
    }
