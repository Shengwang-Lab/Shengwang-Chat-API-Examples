...
window.chatClient.addEventHandler("audioMessage", {
  onAudioMessage: (message) => {
    document
      .getElementById("log")
      .appendChild(document.createElement("div"))
      .append("Message from: " + message.from + " Type: " + message.type);
  },
});
...
document.getElementById("recordBox").onclick = function () {
  let step = document.getElementById("recordBox").textContent.toString().trim();
  if (step === "start recording") {
    ...
    if (recorderObj) {
      ...
      const uri = {
        url: ChatSDK.utils.parseDownloadResponse.call(window.chatClient, blob),
      ...
      window.chatClient
        ...
        .then((res) => {
          ...
          document
            .getElementById("log")
            .appendChild(document.createElement("div"))
            .append("Message send to: " + targetId + " Type: " + msg.type);
        })
        .catch((err) => {
          console.log("send audio fail", err);
        });
    }
  }
};
